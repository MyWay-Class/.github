...
- 작업 지시는 `목표 / 대상 / 완료 기준 / 참조 문서` 순서로 짧게 씁니다.
- 분석 요청은 파일명과 라인 번호를 함께 적습니다.
- 컨벤션은 문서에 두고, 작업 지시에는 경로만 참조합니다.
- 같은 이슈는 필요할 때만 워킹트리로 2안까지 병렬 비교합니다.
- 결과는 바로 믿지 않고, 문서와 타입과 검증 결과를 함께 봅니다.
- 채택한 이유와 변경 요약은 `docs/dev-logs/`에 남깁니다.
