...
[SCOPE]
- 인증 관련 코드만 수정
- Controller / Service / Filter / ExceptionHandler 분리

[TASK]
1. `/login` 로그인 API를 만든다.
2. 로그인 성공 시 JWT를 발급한다.
3. 요청마다 JWT를 검증하는 필터를 구현한다.
4. 인증 실패 시 401을 반환한다.

[CONSTRAINTS]
- 예외 응답 형식은 프로젝트 규칙에 맞춘다.
- 토큰 검증 실패 시 인증 흐름을 중단한다.

[OUTPUT]
- Controller / Service / Filter / 예외 처리 코드 분리

[ACCEPTANCE CRITERIA]
- `/login` 성공 시 JWT가 응답된다.
- 잘못된 토큰이면 401이 반환된다.
- 각 계층이 분리되어 있다.
```
